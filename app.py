...
def main():
    ...
    # nav = option_menu(None, ["Sector-Wise", 'State-Wise'], icons=['briefcase', 'geo-alt'], default_index=0, menu_icon='list', orientation='horizontal')
    ...
    if sector_select == 'All sectors':
        ...
        fig = px.choropleth(states_df, locations='State', locationmode='USA-states', color='Number of Job Postings',
                scope='usa', color_continuous_scale='teal',
        ...
        for state in states:
            try:
                state_counts[state.strip()] = sector_filter_df_copy['job_state'].value_counts()[state]
            except KeyError:
                state_counts[state.strip()] = 0
        states_df = pd.DataFrame.from_dict(state_counts, orient="index", columns=["Number of Job Postings"])
        states_df = states_df.reset_index()
